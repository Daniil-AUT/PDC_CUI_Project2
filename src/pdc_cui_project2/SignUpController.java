...
public class SignUpController {
    ...
    Use switch case statement to identify which field contains errors.
     */
    private void setError(String errorMessage, Field field, boolean error) {
        switch (field) {
            case NAME:
                ...
                break;
            case LNAME:
                setLNameError(errorMessage, error);
                break;
            case EMAIL:
                setEmailError(errorMessage, error);
                break;
            case PASS:
                setPassError(errorMessage, error);
                break;
            case CPASS:
                setCPassError(errorMessage, error);
                break;
        }
    }
    
    // Set error checking method for name field
    private void setNameError(String errorMessage, boolean error) {
        view.nameLabel.setText(error ? "Name (" + errorMessage 
                + "): " : "Name: ");
        view.nameLabel.setForeground(error ? ERROR_COLOUR : DEFAULT_COLOUR);
        view.name.setBorder(new LineBorder(error ? ERROR_COLOUR : 
                DEFAULT_COLOUR, 1));
    }

    // Set error checking method for last name field
    private void setLNameError(String errorMessage, boolean error) {
        view.lName.setText(error ? "Last Name (" + errorMessage 
                + "): " : "Last Name: ");
        view.lName.setForeground(error ? ERROR_COLOUR : DEFAULT_COLOUR);
        view.lastName.setBorder(new LineBorder(error ? ERROR_COLOUR : 
                DEFAULT_COLOUR, 1));
    }
    
    // Set error checking method for email field
    private void setEmailError(String errorMessage, boolean error) {
        view.emailLabel.setText(error ? "Email  (" + errorMessage 
                + "): " : "Email: ");
        view.emailLabel.setForeground(error ? ERROR_COLOUR : DEFAULT_COLOUR);
        view.email.setBorder(new LineBorder(error ? ERROR_COLOUR : 
                DEFAULT_COLOUR, 1));
    }
    
    // Set error checking method for password field
    private void setPassError(String errorMessage, boolean error) {
        view.passLabel.setText(error ? "Password (" + errorMessage 
                + "): " : "Password: ");
        view.passLabel.setForeground(error ? ERROR_COLOUR : DEFAULT_COLOUR);
        view.password.setBorder(new LineBorder(error ? ERROR_COLOUR : 
                DEFAULT_COLOUR, 1));
    }

    // Set error checking method for confirm password field
    private void setCPassError(String errorMessage, boolean error) {
        ...
        view.confPassword.setBorder(new LineBorder(error ? ERROR_COLOUR : 
                DEFAULT_COLOUR, 1));
        setPassError(errorMessage, error);
    }
